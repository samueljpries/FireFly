...
use crate::{
    ...
    drivers::{battery::DummyBattery, imu::DummyImu, motor_output::DummyMotorOutput, rc_input::DummyRcInput},
...
pub async fn run(spawner: Spawner) {
    ...
    let imu = DummyImu::new();
    let battery = DummyBattery::new();
    let rc = DummyRcInput::new();
    let motors = DummyMotorOutput::new();
    ...
    let health = HealthMonitor::new(APP_CONFIG.battery.low_voltage_v, APP_CONFIG.battery.critical_voltage_v);
    ...
    spawner.spawn(tasks::sensor_task::run(imu, battery, rc, sensor_snapshot, pilot_command)).ok();
    spawner.spawn(tasks::control_task::run(
        estimator,
        commander,
        attitude,
        rate,
    ...
    spawner.spawn(tasks::motor_task::run(motors, motor_command)).ok();
    spawner.spawn(tasks::telemetry_task::run(vehicle_state, pilot_command, motor_command)).ok();
    spawner.spawn(tasks::safety_task::run(health, sensor_snapshot, vehicle_state, motor_command)).ok();
